...
{
	public partial class Invoice_WithholdingTaxSubtotal
	{
		...
		public bool Load()
		{
			...
			cmd.CommandText = "SELECT * FROM Invoice_WithholdingTaxSubtotal WHERE nID = " + this.nID;
		...
		public bool Save()
		{
			...
			if (nID == 0)
			{
				cmd.CommandText = "INSERT INTO Invoice_WithholdingTaxSubtotal (InvoiceID, TaxableAmount, currencyID, TaxAmount, PerUnitAmount, CalculationSequenceNumeric, Percent_, TaxCategory_TaxScheme_Name, TaxCategory_TaxScheme_TaxTypeCode, TaxCategory_TaxExemptionReason, TaxCategory_TaxExemptionReasonCode) "; 
				cmd.CommandText += " VALUES (@InvoiceID, @TaxableAmount, @currencyID, @TaxAmount, @PerUnitAmount, @CalculationSequenceNumeric, @Percent_, @TaxCategory_TaxScheme_Name, @TaxCategory_TaxScheme_TaxTypeCode, @TaxCategory_TaxExemptionReason, @TaxCategory_TaxExemptionReasonCode)"; 
			...
			{
				cmd.CommandText = "UPDATE Invoice_WithholdingTaxSubtotal SET InvoiceID=@InvoiceID, TaxableAmount=@TaxableAmount, currencyID=@currencyID, TaxAmount=@TaxAmount, PerUnitAmount=@PerUnitAmount, CalculationSequenceNumeric=@CalculationSequenceNumeric, Percent_=@Percent_, TaxCategory_TaxScheme_Name=@TaxCategory_TaxScheme_Name, TaxCategory_TaxScheme_TaxTypeCode=@TaxCategory_TaxScheme_TaxTypeCode, TaxCategory_TaxExemptionReason=@TaxCategory_TaxExemptionReason, TaxCategory_TaxExemptionReasonCode=@TaxCategory_TaxExemptionReasonCode ";
