...
namespace MrTEK_EFatura
{
	public partial class Invoice_Notes
	{
		...
		public bool Load()
		{
			...
			cmd.CommandText = "SELECT * FROM Invoice_Notes WHERE nID = " + this.nID;
		...
		public bool Save()
		{
			...
			if (nID == 0)
			{
				cmd.CommandText = "INSERT INTO Invoice_Notes (InvoiceID, Note) "; 
			...
			{
				cmd.CommandText = "UPDATE Invoice_Notes SET InvoiceID=@InvoiceID, Note=@Note ";
