...
{
	public partial class InvoiceLine_WithholdingTaxSubtotal
	{
		...
		public bool Load()
		{
			...
			cmd.CommandText = "SELECT * FROM InvoiceLine_WithholdingTaxSubtotal WHERE nID = " + this.nID;
		...
		public bool Save()
		{
			...
			if (nID == 0)
			{
				cmd.CommandText = "INSERT INTO InvoiceLine_WithholdingTaxSubtotal (InvoiceLineID, TaxableAmount, currencyID, TaxAmount, PerUnitAmount, CalculationSequenceNumeric, Percent_, TaxCategory_TaxScheme_Name, TaxCategory_TaxScheme_TaxTypeCode, TaxCategory_TaxExemptionReason, TaxCategory_TaxExemptionReasonCode) "; 
